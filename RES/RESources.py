...
from RES.step_cache import StepCache
...
class RESources_builder(AttributesParser):
    ...
    def find_grid_nodes(self,
        ...
        """
        self.cutout, self.region_boundary = self.era5_cutout.get_era5_cutout(weather_year=self.weather_year)
    ...
    def build(self,
        ...
        if cache.is_current("grid_cells", "cells") and cache.is_current("grid_nodes", "cells"):
            utils.print_update(level=2, message="Step 1: grid config unchanged — loading from store.")
        else:
            self.get_grid_cells()
        ...
        if cache.is_current("cell_capacity", "cells"):
            utils.print_update(level=2, message="Step 2: capacity config unchanged — loading from store.")
        else:
        ...
        if cache.is_current("weather_data", "cells") and cache.is_current("gwa_scaling", "cells"):
            utils.print_update(level=2, message="Step 3: weather config unchanged — loading from store.")
        else:
        ...
        if cache.is_current("cf_timeseries", ts_store_key):
            utils.print_update(level=2, message="Step 4: cutout/turbine config unchanged — loading from store.")
        else:
        ...
        if cache.is_current("scoring", "cells"):
            utils.print_update(level=2, message="Step 5: economic config unchanged — loading from store.")
            self.datahandler.refresh()
        ...
        if make_clusters:
            ...
            if cache.is_current("clustering", cluster_store_key):
                utils.print_update(level=2, message="Step 6: cluster config unchanged — loading from store.")
        ...
        if select_top_sites:
            ...
            if cache.is_current("clustering", cluster_store_key) and self.clusters_nt is not None:
                utils.print_update(level=2, message="Step 7: cluster config unchanged — using cached clusters.")
            else:
                self.get_clusters()
                cache.mark_done("clustering")
