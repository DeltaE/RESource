...
try:
    ...
    gpd = None
    warnings.warn("geopandas not found. Clip-to-boundary and boundary auto-build will be disabled.", RuntimeWarning)

# ----------------------------
# Repo paths
# ----------------------------
REPO_ROOT = Path(__file__).resolve().parents[1]
DL_ROOT   = REPO_ROOT / "data" / "downloaded_data" / "EU_DEM"
PROC_ROOT = REPO_ROOT / "data" / "processed_data" / "EU_DEM"
...
def gdal_mosaic(tifs: List[Path], out_path: Path) -> Path:
    ...
    subprocess.run(
        ["gdal_translate", "-co", "COMPRESS=LZW", "-co", "BIGTIFF=YES", str(vrt_path), str(out_path)],
        check=True,
        stdout=subprocess.PIPE,
        stderr=subprocess.PIPE,
    )
...
# ---------- Clip ----------
def clip_to_boundary(in_path: Path, boundary_path: Optional[Path], out_path: Path) -> Path:
    ...
    if gpd is None:
        warnings.warn("geopandas not installed; skipping clip.", RuntimeWarning)
        if in_path != out_path:
            shutil.copyfile(in_path, out_path)
        return out_path
...
def compute_slope_percent_from_dem(in_path: Path, out_path: Path) -> Path:
    ...
    with rasterio.open(in_path) as src:
        ...
        kx = np.array([[-1, 0, 1],
                       [-2, 0, 2],
                       [-1, 0, 1]], dtype="float32") / (8.0 * px)
        ky = np.array([[ 1,  2,  1],
                       [ 0,  0,  0],
                       [-1, -2, -1]], dtype="float32") / (8.0 * py)
...
def reclass_slope_to_bins(
    in_path: Path,
    out_path: Path,
    bins: List[float] = [0.5, 2, 5, 8, 16, 30, 45, 1e9],
...
def make_wb6_boundary(
    ...
    if not files:
        print(f"[WARN] No inputs matching '{pattern}' in {regions_dir} — cannot auto-build boundary.", file=sys.stderr)
        return None
...
def main():
    ...
    parser.add_argument("--use-copdem30-wb6", action="store_true",
    ...
    parser.add_argument("--auto-boundary-from", type=str, default=str(REPO_ROOT / "data" / "processed_data" / "regions"),
                        help="Directory with per-country WB6 GeoJSONs to auto-build a dissolved boundary (if needed).")
    parser.add_argument("--auto-boundary-pattern", type=str, default="gadm41_Western Balkan Regions_L1_*.geojson",
                        help="Glob pattern for input country GeoJSONs.")
    parser.add_argument("--auto-boundary-out", type=str, default="WB6_laea3035.geojson",
                        help="Output filename for dissolved WB6 boundary (EPSG:3035).")
    parser.add_argument("--force-make-boundary", action="store_true",
                        help="Force rebuild of the dissolved boundary even if it exists.")
    parser.add_argument("--target-res", type=float, default=30.0,
                        help="Target metric resolution for DEM reprojection (meters). Default 30 (GLO-30).")
    parser.add_argument("--make-100m", action="store_true",
                        help="Also create a 100 m slope (%).")
    parser.add_argument("--water-mask", type=str, default=None,
    ...
    parser.add_argument("--clean-tiles", action="store_true",
                        help="Delete existing .tif in tiles/ before downloading new ones.")

    # NEW: prefer/force GDAL for reproject & slope + debug
    parser.add_argument("--force-gdal", action="store_true",
                        help="Force use of GDAL CLI (gdalwarp/gdaldem) for reprojection and slope if available.")
    parser.add_argument("--debug-gdal", action="store_true",
                        help="Print which GDAL executables are detected.")
    ...
    if boundary_path is None:
        ...
        if maybe and maybe.exists():
            ...
            print("[WARN] Proceeding without a WB6 boundary; will use coarse bbox and skip clipping.", file=sys.stderr)
    ...
    if args.use_copdem30_wb6:
        if boundary_path and boundary_path.exists():
            ...
            urls.extend(copdem30_urls_from_bbox(lat_min=39, lat_max=47, lon_min=15, lon_max=24))  # slightly expanded south
    ...
    if not tifs:
        print(f"[ERROR] No GeoTIFF tiles selected in {TILES_DIR}. Provide tiles or URLs.", file=sys.stderr)
        sys.exit(2)
    else:
    ...
    if _have_gdal_cli():
        ...
        print("[WARN] GDAL CLI not found; falling back to in-memory rasterio.merge (slower, high RAM).")
    ...
    # Slope
    slope_native_path = SLOPE_25 if args.target_res <= 25.0 else PROC_ROOT / f"WB6_slope_pct_{int(args.target_res)}m.tif"
