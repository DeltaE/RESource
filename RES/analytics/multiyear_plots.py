...
def fig_rank_stability(ranks: pd.DataFrame, out_dir) -> None:
    ...
    for ax, tech in zip(axes, TECHS):
        ...
        if "lcoe_spearman" in sub.columns:
            ax.plot(sub["year"], sub["lcoe_spearman"],
                    color=col, lw=2, marker="o", ms=6,
                    label="LCOE proxy ranking", zorder=4)
        ...
        if "cf_spearman" in sub.columns:
            ax.plot(sub["year"], sub["cf_spearman"],
                    color=col, lw=1.5, marker="s", ms=5,
                    ls="--", alpha=0.75,
                    label="CF ranking", zorder=3)

        ax.axhline(1.0,  color="#aaa", lw=0.8, ls=":")
        ax.axhline(0.95, color="#e63946", lw=0.9, ls="--",
                   label="ρ = 0.95 reference", zorder=2)
        ax.axvline(ref,  color="#555", lw=0.8, ls=":",
                   label=f"Reference year ({ref})")

        ax.set_xlim(sub["year"].min() - 0.5, sub["year"].max() + 0.5)
        ax.set_ylim(0.70, 1.02)
        ax.xaxis.set_major_locator(mticker.MaxNLocator(integer=True))
        ax.set_xlabel("Weather year", fontsize=10)
        ax.set_title(f"{tech.title()} — cell rank stability vs {ref}",
    ...
    fig.suptitle("Interannual rank stability of cell-level LCOE proxy and CF — BC",
                 fontsize=11, fontweight="bold", y=1.01)
    fig.tight_layout()
    fig.savefig(out_dir / "fig_M4-1_rank_stability.png",
                dpi=FIG_DPI, bbox_inches="tight")
    plt.show()
    print(f"  Saved fig_M4-1_rank_stability.png")
...
def fig_accessible_capacity(long_df: pd.DataFrame,
                            lcoe_thresh: dict,
                            out_dir) -> None:
    """
    ...
    for y in years:
        ...
        for t in TECHS:
            ...
            if lc not in dy.columns or cap not in dy.columns:
                records[t].append(np.nan); continue
    ...
    for i, tech in enumerate(TECHS):
        ...
        bars = ax.bar(x + (i - 0.5) * width, vals, width,
                      color=TECH_COLOR[tech], alpha=0.85,
    ...
    for tech in TECHS:
        ...
        ax.axhline(mean_v, color=TECH_COLOR[tech], lw=1.2,
                   ls="--", alpha=0.7)
        ax.text(x[-1] + 0.55, mean_v, f"μ={mean_v:.1f}",
                color=TECH_COLOR[tech], fontsize=7.5, va="center")

    ax.set_xticks(x)
    ax.set_xticklabels(years, fontsize=9)
    ax.set_xlabel("Weather year", fontsize=10)
    ax.set_ylabel("Accessible capacity  (GW)", fontsize=10)
    ax.set_title("Interannual variation in accessible capacity — BC",
                 fontweight="bold", fontsize=11)
    ax.legend(fontsize=9, frameon=False)
    _spines(ax)
    ax.grid(axis="y", ls="--", lw=0.4, alpha=0.4)
    fig.tight_layout()
    fig.savefig(out_dir / "fig_M4-2_accessible_capacity_by_year.png",
                dpi=FIG_DPI, bbox_inches="tight")
    plt.show()
    print(f"  Saved fig_M4-2_accessible_capacity_by_year.png")
...
def fig_cf_anomaly_heatmap(long_df: pd.DataFrame, out_dir) -> None:
    ...
    for ax, tech in zip(axes, TECHS):
        ...
        df["decile"] = pd.qcut(df["cf_mean_all"], 10,
                                labels=[f"D{i}" for i in range(1, 11)])

        pivot = df.pivot_table(index="decile", columns="year",
        ...
        ax.set_title(f"{tech.title()} — CF anomaly by decile and year",
                     fontweight="bold", fontsize=10)
        ...
        for i in range(len(pivot.index)):
            for j in range(len(pivot.columns)):
                v = pivot.values[i, j]
                if np.isfinite(v):
                    lum = 0.2126 * 1 + 0.7152 * 1 + 0.0722 * 1  # white bg
                    ax.text(j, i, f"{v:+.2f}",
                            ha="center", va="center",
                            fontsize=5.5,
                            color="white" if abs(v) > vabs * 0.5 else "#333")

    fig.suptitle("Cell-level CF anomaly by decile group — BC",
                 fontsize=11, fontweight="bold", y=1.01)
    fig.tight_layout()
    fig.savefig(out_dir / "fig_M4-3_cf_anomaly_heatmap.png",
                dpi=FIG_DPI, bbox_inches="tight")
    plt.show()
    print(f"  Saved fig_M4-3_cf_anomaly_heatmap.png")
...
def fig_cf_mean_vs_cv(stats: pd.DataFrame, out_dir) -> None:
    ...
    for ax, tech in zip(axes, TECHS):
        ...
        # Hex-bin density
        hb = ax.hexbin(x, y, gridsize=35, cmap="YlOrRd",
        ...
        ax.plot(xs, np.polyval(z, xs),
                color=TECH_COLOR[tech], lw=1.5, ls="--",
        ...
        ax.set_title(f"{tech.title()} — productivity vs stability tradeoff",
                     fontweight="bold", fontsize=10)
        ax.legend(fontsize=8, frameon=False)
        _spines(ax)
        ax.grid(ls="--", lw=0.4, alpha=0.4)

    fig.suptitle("Per-cell CF mean vs interannual CV — BC  (n = 2,956 cells)",
                 fontsize=11, fontweight="bold", y=1.01)
    fig.tight_layout()
    fig.savefig(out_dir / "fig_M4-4_cf_mean_vs_cv_scatter.png",
                dpi=FIG_DPI, bbox_inches="tight")
    plt.show()
    print(f"  Saved fig_M4-4_cf_mean_vs_cv_scatter.png")
...
def fig_lcoe_cv_violin(stats: pd.DataFrame, out_dir) -> None:
    ...
    for el in ["whiskers", "caps", "medians"]:
        for line in bp[el]:
            line.set_color("#333"); line.set_linewidth(1.2)

    ax.set_xticks(range(len(labels)))
    ax.set_xticklabels(labels, fontsize=11)
    ax.set_ylabel("LCOE coefficient of variation  (%)", fontsize=10)
    ax.set_title("Interannual LCOE proxy uncertainty — BC  (per-cell CV)",
                 fontweight="bold", fontsize=11)
    _spines(ax)
    ax.grid(axis="y", ls="--", lw=0.4, alpha=0.4)
    fig.tight_layout()
    fig.savefig(out_dir / "fig_M4-5_lcoe_cv_violin.png",
                dpi=FIG_DPI, bbox_inches="tight")
    plt.show()
    print(f"  Saved fig_M4-5_lcoe_cv_violin.png")
...
def fig_pairwise_spearman(long_df: pd.DataFrame, out_dir) -> None:
    ...
    for ax, tech in zip(axes, TECHS):
        ...
        im = ax.imshow(mat_disp, cmap="RdYlGn",
                       vmin=max(0.80, vmin), vmax=1.0,
                       aspect="auto")
        cbar = plt.colorbar(im, ax=ax, shrink=0.85, pad=0.02)
        cbar.set_label("Spearman ρ  (LCOE ranking)", fontsize=8)
        cbar.ax.tick_params(labelsize=7)

        ax.set_xticks(range(n)); ax.set_xticklabels(years, rotation=45,
                                                     ha="right", fontsize=8)
        ax.set_yticks(range(n)); ax.set_yticklabels(years, fontsize=8)
        ...
        ax.set_title(f"{tech.title()} — pairwise LCOE rank correlation",
                     fontweight="bold", fontsize=10)

    fig.suptitle("Pairwise Spearman ρ of cell LCOE ranking — BC  (all year-pairs)",
                 fontsize=11, fontweight="bold", y=1.01)
    fig.tight_layout()
    fig.savefig(out_dir / "fig_M4-6_pairwise_spearman_matrix.png",
                dpi=FIG_DPI, bbox_inches="tight")
    plt.show()
    print(f"  Saved fig_M4-6_pairwise_spearman_matrix.png")
...
if __name__ == "__main__":
    ...
    for y in years:
        ...
        rows.extend([
            {"cell_id": i, "year": y,
             "solar_CF_mean": cf_s[i], "wind_CF_mean": cf_w[i],
    ...
    for tech in TECHS:
        lc = LCOE_COL[tech]
        ref_df = long_df[long_df["year"] == 2024 if 2024 in years else years[-1]].set_index("cell_id")
        for y in years:
            ...
            if len(common) > 2 and lc in long_df.columns:
                ...
                ranks_rows.append({"tech": tech, "ref_year": years[-1], "year": y,
                                   "lcoe_spearman": rho, "cf_spearman": rho - 0.01})
    ranks = pd.DataFrame(ranks_rows)

    out = Path("/tmp/test_figs")
    out.mkdir(exist_ok=True)
    make_extended_figures(stats, long_df, ranks,
                          {"solar": 90, "wind": 130}, out)
