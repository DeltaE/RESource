...
# %% [markdown]
# ## iii. Global Administrative (GADM) Regions [for regional boundaries]
...
# %% [markdown]
# * Prepare Regional Data (admin level=2)

# %%
def prepare_GADM_data(
        country:str,
        current_region:dict,
        datafile_save_to:str,
        plot_save_to:str,
        ad_level:int=2,
        ):
    """ 
    Takes the COUNTRY, REGION, ADMINISTRATIVE REGION LEVEL inputs to trim the GADM regional boundaries dataset and saves the datafile locally as ".geojson",
    also creates a visual of the regions.
    """

    province:str=current_region['name']
    province_code:str=current_region['code']
    region_mapping:dict=current_region['region_mapping']
    # plt_save_to:str=datafile_save_to.replace("geojson","png")

    # CAN_gadm_regions_gdf = downloader.get_shape_data_by_country_name(country_name=country_name, ad_level=ad_level) # V4.0 with GADM downloader API
    CAN_gadm_regions_gdf = pygadm.Items(name=country,content_level=ad_level) # V4.1 with pyGADM downloader API

    if isinstance(CAN_gadm_regions_gdf, gpd.GeoDataFrame):

        province_gadm_gdf = CAN_gadm_regions_gdf[CAN_gadm_regions_gdf['NAME_1'] == province]

        # bc_gadm_regions_gdf=bc_gadm_gdf.loc[:,['COUNTRY', 'NAME_1','NAME_2','geometry']] #GADM downloader file
        province_gadm_regions_gdf=province_gadm_gdf.loc[:,['NAME_0','NAME_1','NAME_2','geometry']]  #PyGADM file
        province_gadm_gdf = {
            'NAME_0': 'COUNTRY', #Not in pyGADM file
            'NAME_1': 'Province',
            'NAME_2': 'Region'
        }

        province_gadm_regions_gdf = province_gadm_regions_gdf.rename(columns=province_gadm_gdf)

        # Map the region names to Map IDs and create a new 'Map_ID' column
        province_gadm_regions_gdf['Region_ID'] = province_gadm_regions_gdf['Region'].map(region_mapping)
        
        province_gadm_regions_gdf.sort_values(by='Region_ID', inplace=True)
        province_gadm_regions_gdf.reset_index(inplace=True)
        province_gadm_regions_gdf.drop(columns='index',inplace=True)
        province_gadm_regions_gdf['Region'].astype('category')
        province_gadm_regions_gdf.crs=4326
        province_gadm_regions_gdf.to_file(datafile_save_to,driver='GeoJSON') #save the local file as geojson
        
        log.info(f"GADM (level={ad_level}) BC Boundary loaded. No. of Regions : {len(province_gadm_regions_gdf)}")
        utils.check_LocalCopy_and_run_function(plot_save_to,lambda:visualize_GADM_regions(province_gadm_regions_gdf,"deep",plot_save_to),force_update=False)
       
    else:
        log.warning(">> Failed to download or process GADM data.Updating the data sourcing guideline for manual download.\n")


# %% [markdown]
# ## iv. Canadian Open Data for Energy Resources (CODERS) [for existing system data]
...
# %% [markdown]
# ## vi. Bounding Box 
# >(Minimum Bounding Rectangle coordinates from GADM Boundary)

# %%
def plot_n_save_bounding_box(
        province_gadm_regions_gdf: gpd.GeoDataFrame,
        current_region:dict,
        bounding_box_save_to: str
        ):
    
    if current_region['cutout_bounding_box']['automated']:
        log.info(f"Calculating the Minimum Bounding Region (MBR) for {current_region['name']}")
        min_x, min_y, max_x, max_y=province_gadm_regions_gdf.geometry.total_bounds
        ...
        log.info(f"User defined Minimum Bounding Region (MBR) Selected for {current_region['name']}")
        bounding_box:dict = current_region['cutout_bounding_box']['user_defined']
        min_x, max_x, min_y, max_y = bounding_box.values()
        plot_info='(user defined)'

    # Create a GeoDataFrame for the MBR
    bounding_box_gdf = gpd.GeoDataFrame(geometry=[box(min_x, min_y, max_x, max_y)], crs=province_gadm_regions_gdf.crs)

    # Plotting
    fig, ax = plt.subplots()
    province_gadm_regions_gdf.plot(ax=ax, color='skyblue')
    bounding_box_gdf.boundary.plot(ax=ax, color='red')

    # Define the text for the legend box
    legend_text = f'min_x: {min_x:.2f}\nmax_x: {max_x:.2f}\nmin_y: {min_y:.2f}\nmax_y: {max_y:.2f}'

    # Add a legend box outside the plot
    ax.legend([legend_text], loc='upper left', bbox_to_anchor=(1.001, 1), fontsize=8, title='MBR Coordinates', title_fontsize='medium')

    plt.title(f"Bounding Box {plot_info} for the Cutout")
    plt.grid(True, ls="--", linewidth=0.7)
    plt.savefig(f'vis/misc/{current_region['code']}_MBR'.replace(".geojson", ".png"),dpi=300)

    # Save the box to a new file (optional)
    bounding_box_gdf.to_file(bounding_box_save_to, driver='GeoJSON')
    log.info(f"Geospatial Bounding Box Created for {current_region['code']} Province")
    return bounding_box
# %% [markdown]
...
def CLIP_n_PLOT_GAEZraster_for_province(
    ...
    raster_plot_name=f'{current_region['code']}_{raster}'
    raster_dump_direct=os.path.join(parent_direct,Rasters_in_use_direct,zip_direct,current_region['code'])
    ...
    with rasterio.open(ip_raster_file) as src:
        ...
        create_raster_image_with_legend(output_clipped_raster_path,cmap,type,plot_save_to)
...
def prepare_GAEZ_raster_files(
    ...
    for type,raster,zip_direct,cmap in zip(raster_types,raster_filenames,zip_extract_directories,color_map):
        CLIP_n_PLOT_GAEZraster_for_province(current_region,root_dir,Rasters_in_use_direct,type,zip_direct,raster,cmap,gadm_boundary_buffer.geometry)

    return log.info ("Required rasters for GAEZ processed and plotted successfully !")

# %% [markdown]
# ## viii. Canadian Protected and Conserved Areas Database (CPCAD) 
# > [for local exclusion areas with.without buffer zones] 
# %% [markdown]
# * Pull data as ZIP

# # %%
# def download_gov_protected_areas_data_resources(
#     source_URL:str,
#     file_path:str):
...
#         print(f"Please Download the data from {source_URL} and extract the files inside zip to {file_path}")
#         print(f"Please donwload the 'ProtectedConservedArea_2022.gdb.zip' from Canadian Protected and Conserved Areas Database (CPCAD) - Protected and conserved area database: https://open.canada.ca/data/dataset/6c343726-1e92-451a-876a-76e17d398a1c/resource/672d5acf-5309-47af-9775-5994b2e238b9")
#     # return file_path

# %% [markdown]
# * Extract required region's data
...
def extract_provincial_data_from_gov_conservation_lands(
    ...
    gdb_file_name = gov_conservation_lands['geodatabase_file']
    data_root=gov_conservation_lands['data_root']#.replace('data',"data/downloaded_data")
    gdb_file_path=os.path.join(data_root,gdb_file_name)
    zip_file_path=str(gdb_file_path+'.zip')

    # Directory to extract the contents of the zip file
    extraction_direct = gov_conservation_lands['zip_expand'].replace('data',"data/downloaded_data")
    ...
    conservation_lands_provincial_id=current_region['conservation_lands_provincial_id']
