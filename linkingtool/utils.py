...
def get_gen_bus(node_code, bus_dict, province='BC'):
    '''
    ...
    return -> "230_ABS_GSS"
    '''
    if (province == 'SK') or (province == 'MB') or (province == 'BC'):
        return province
    else:
...
def add_generic_columns(assets, gen_generic, gen_type):
    ...
    mask = gen_generic['generation_type'] == gen_type # "Wind_onshore"
...
def add_generic_columns_tpp(assets, gen_generic):
    ...
    df_new = pd.merge(assets, gen_generic[columns_to_add + ['generation_type']], on='generation_type', how='left')
