...
>>> from RESource.cluster import cells_to_cluster_mapping, create_cells_Union_in_clusters
>>>
...
def assign_cluster_id(
    ...
    if source_column is None:
        raise ValueError(
            f"'{source_column}' not defined for indexing. Please provide a valid source column name."
        )
...
def find_optimal_K(
    ...
    optimal_k_data = next(
        (k for k, wcss_value in enumerate(wcss_data, start=1) if wcss_value <= tolerance_data), None
    )
    ...
    # Print the optimal k
    print(
        f"Zone {region} - Optimal k for LCOE_{resource_type} based clustering: {optimal_k_data}\n"
    )
...
def pre_process_cluster_mapping(
    ...
    try:
        ...
        raise ValueError(
            f"Failed to create directory at {elbow_plot_directory}. Ensure 'vis_directory' is valid. Error: {e}"
        ) from e
    ...
    for region in unique_regions:
        ...
        data_for_clustering = cells_scored[cells_scored[sub_national_unit_tag] == region][
            expected_cols
        ]
        ...
        data_for_clustering_cleaned = pd.DataFrame(
            imputed_array, columns=data_for_clustering.columns
        )

        # Call the function for K-means clustering and elbow plot
        optimal_k = find_optimal_K(
            resource_type, data_for_clustering_cleaned, region, wcss_tolerance, max_k=15
        )
    ...
    print(
        f"Optimal-k based on 'LCOE' clustering calculated for {len(unique_regions)} zones and saved to cell dataframe.\n"
    )
...
def cells_to_cluster_mapping(
    ...
    """
    dataframe, optimal_k_df = pre_process_cluster_mapping(
        cells_scored, vis_directory, wcss_tolerance, sub_national_unit_tag, resource_type
    )

    utils.print_update(
        level=2, message="Mapping the Optimal Number of Clusters for Each region ..."
    )

    clusters = []
    dataframe_filtered = dataframe[
        dataframe[sub_national_unit_tag].isin(list(optimal_k_df[sub_national_unit_tag]))
    ]
    ...
    for region, group in dataframe_filtered.groupby(sub_national_unit_tag):
        ...
        clusters.extend(
            [
                group.iloc[i : i + region_step_size].copy()
                for i in range(0, region_rows, region_step_size)
            ]
        )
...
def create_cells_Union_in_clusters(
    ...
    **Classification**:
    ...
    node_distance_col = utils.get_available_column(
        cluster_map_gdf, ["nearest_station_distance_km", "nearest_distance"]
    )
    grid_node_col = utils.get_available_column(
        cluster_map_gdf, ["nearest_station", "nearest_connection_point"]
    )
    ...
    for region in region_optimal_k_df[sub_national_unit_tag]:
        i += 1
        log.info(
            f" Creating cluster for {region} {i}/{len(region_optimal_k_df[sub_national_unit_tag])}"
        )
        ...
        dissolved_gdf = utils.assign_regional_cell_ids(
            dissolved_gdf, sub_national_unit_tag, "cluster_id"
        )
        ...
        dissolved_gdf.columns = dissolved_gdf.columns.str.replace(
            rf"(?i)(_{resource_type}|{resource_type}_)", "", regex=True
        )

    utils.print_update(
        level=2,
        message="Clusters Created and a list generated to map the Cells inside each Cluster...",
    )
    return dissolved_gdf, dissolved_indices


def clip_cluster_boundaries_upto_regions(
    cell_cluster_gdf: gpd.GeoDataFrame, gadm_regions_gdf: gpd.GeoDataFrame, resource_type
) -> gpd.GeoDataFrame:
    """
    Clip cluster boundaries to precise regional administrative boundaries.
