...
import RESource.windspeed as windspeed
...
from RESource.tech import OEDBTurbines
...
class Timeseries(AttributesParser):
    ...
    def get_timeseries(self, cells: gpd.GeoDataFrame, weather_year: int | None = None) -> tuple:
        """
        ...
        if self.resource_type == "solar":
            ...
            self.sites_profile: xr.DataArray = self.__process_PV_timeseries__(
                cells, weather_year=weather_year
            )

        elif self.resource_type == "wind":
            # Step 1: Set-up Technology parameters and extract the synthetic timeseries data for all sites
            self.sites_profile: xr.DataArray = self.__process_WIND_timeseries__(
                cells, "OEDB", 2, weather_year=weather_year
            )
        ...
        self._CF_ts_df_org: pd.DataFrame = self.sites_profile.to_dataframe().unstack(
            "cell"
        )  # Multi-index dataframe with Y_L1 index (array name i.e. "solar" or "wind")
        self._CF_ts_df_ = self._CF_ts_df_org.loc[
            :, self.resource_type
        ]  # Multi-index dataframe with Y_L1 index (array name i.e. "solar" or "wind")
        ...
        utils.print_update(
            ...
            message=f"{__name__}| Calculating CF mean from the {len(self.CF_ts_df)} data points for each Cell ...",
        )
        utils.print_update(
            level=PRINT_LEVEL_BASE + 2, message=f"{__name__}| Total Grid Cells: {len(cells)}"
        )
        utils.print_update(
            level=PRINT_LEVEL_BASE + 2,
            message=f"{__name__}| Timeseries Generated for: {len(self.CF_ts_df.columns)}",
        )
        utils.print_update(
            level=PRINT_LEVEL_BASE + 2,
            message=f"{__name__}| Matched Sites: {self.CF_ts_df[cells.index].shape}",
        )

        cells[f"{self.resource_type}_CF_mean"] = self.CF_ts_df.mean(
            axis=0
        )  # Mean of all rows (Hours)
        ...
        self.datahandler.to_store(
            cells, "cells"
        )  # We don't want 'force-update' here, just need to append 'CF_mean' data to cells.
        self.datahandler.to_store(
            self.CF_ts_df, f"timeseries/{self.resource_type}"
        )  # Hierarchical data of resources under key 'timeseries'

        return cells, self.CF_ts_df

    def __process_PV_timeseries__(self, cells: gpd.GeoDataFrame, weather_year: int | None = None):
        """
        ...
        utils.print_update(
            level=PRINT_LEVEL_BASE + 1, message=f"{__name__}| Loading technology attributes..."
        )
        ...
        utils.print_update(
            level=PRINT_LEVEL_BASE + 1,
            message=f"{__name__}| ⏳Processing timeseries from ERA5 cutout, may take a while...",
        )
        self.pv_profile: xr.DataArray = self.cutout.pv(**pv_args).rename(self.resource_type)

        return self.pv_profile

    def get_gwa_geogson_data(self, gwa_geojson_file_path: str | Path = None):
        """
        ...
        if not gwa_geojson_file_path.exists():
            utils.print_update(
                level=2,
                message=f"Global Wind Atlas Regional GIS data file not found at: {gwa_geojson_file_path}",
            )
            utils.print_update(
                level=3,
                message="Please download the Global Wind Atlas GIS map data for your region from: https://globalwindatlas.info/en/download/maps-country-and-region",
            )
        else:
    ...
    def get_windatlas_data(self, gwa_windspeed_raster_path: str | Path = None):
        """
    ...
    def __process_WIND_timeseries__(
        self,
        ...
        utils.print_update(
            level=PRINT_LEVEL_BASE, message=f">> {len(cells)} Grid Cells from Store Cutout"
        )
        ...
        utils.print_update(
            level=PRINT_LEVEL_BASE + 1,
            message=f"{__name__}| Rescaling ERA5 windspeed with GWA windspeed",
        )
        ...
        if turbine_model_source == "atlite":
            atlite_turbine_model: str = self.wind_turbine_config[turbine_model_source][model][
                "name"
            ]  # The default is set in Attributes parser's .get_turbine_config() method.
            hub_height_turbine = atlite.resource.get_windturbineconfig(atlite_turbine_model)[
                "hub_height"
            ]

            self.turbine_config: dict = atlite.resource.get_windturbineconfig(
                atlite_turbine_model, {"hub_height": 100}
            )
            utils.print_update(
                level=PRINT_LEVEL_BASE + 2,
                message=f"{__name__}| Selected Wind Turbine  Model : {atlite_turbine_model} @ {hub_height_turbine}m Hub Height",
            )
    ...
    def __fix_timezone__(self, data: pd.DataFrame) -> pd.DataFrame:
        """
        ...
        utils.print_update(
            level=PRINT_LEVEL_BASE + 1,
            message=f"{__name__}| Harmonizing timezone for {self.region_name} with {self.region_timezone}",
        )
    ...
    def get_cluster_timeseries(
        ...
        if results:
            ...
            utils.print_update(
                level=PRINT_LEVEL_BASE + 1,
                message=f"{__name__}| ⚠️ No valid clusters to process for cluster {cluster}.",
            )
            self.cluster_df = pd.DataFrame()

        self.datahandler.to_store(
            self.cluster_df, f"timeseries/clusters/{self.resource_type}", force_update=True
        )  # Hierarchical data of resources under kley 'timeseries'
...
def get_timeseries_for_project_points(
    resources_store: Path, projects: gpd.GeoDataFrame, save_to: str | Path, show: bool = True
):
    """
    ...
    utils.print_update(
        level=PRINT_LEVEL_BASE + 1,
        message=f"{__name__}| Saved solar time series data to {solar_ts_save_to}",
    )

    wind_ts.to_csv(wind_ts_save_to)
    utils.print_update(
        level=PRINT_LEVEL_BASE + 1,
        message=f"{__name__}| Saved wind time series data to {wind_ts_save_to}",
    )
    ...
    utils.print_update(
        level=PRINT_LEVEL_BASE + 1,
        message=f"{__name__}| Saved solar time series data to {save_to / 'projects_solar_ts.html'}",
    )
    ...
    utils.print_update(
        level=PRINT_LEVEL_BASE + 1,
        message=f"{__name__}| Saved wind time series data to {save_to / 'projects_wind_ts.html'}",
    )
