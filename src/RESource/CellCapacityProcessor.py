...
from RESource.atb import NREL_ATBProcessor
...
class CellCapacityProcessor(AttributesParser):
    ...
    def __get_unified_region_shape__(self):
        ...
        self.region_shape = self.LandContainer.region_shape.dissolve(
            by=self.gadm_config.get("datafield_mapping").get("NAME_0")
        )  # .drop(columns =['Region'])
    ...
    def load_cost(self, resource_atb: pd.DataFrame = None) -> dict:
        ...
        def _get_atb_value(param_name: str):
            ...
            try:
                ...
                raise ValueError(
                    f"Could not extract ATB value for '{param_name}' for {self.resource_type}."
                ) from e
    ...
    def get_shapes_for_availability(self, cells: gpd.geodataframe = None):
    ...
    def get_capacity(self) -> tuple:
        ...
        """
        utils.print_update(
            level=PRINT_LEVEL_BASE + 1, message=f"{__name__}| Cell capacity processor initiated..."
        )
        ...
        cost_components: dict = self.load_cost(
            resource_atb=(
                ...
                else self.land_based_wind_cost
        ...
        utils.print_update(
            level=PRINT_LEVEL_BASE + 1, message=f"{__name__}| Processing Availability Matrix... "
        )
        ...
        self.AvailabilityMatrix: xr = self.cutout.availabilitymatrix(
            self.shapes_for_availability, self.composite_excluder
        )
        ...
        parameters_to_add = {
            ...
            "Operational_life": self.resource_disaggregation_config.get(
                "operational_life", 20
            ),  # years
        }
        ...
        stylized_columns = {
            f"{key}_{self.resource_type}": value for key, value in parameters_to_add.items()
        }
    ...
    def get_actual_capacity(self, gdf: gpd.GeoDataFrame = None):
        """
    ...
    def get_availability_gdf(self, AvailabilityMatrix: xr = None):
        ...
        self.grid_cells = self.datahandler.from_store("cells")
        if (
            avail_col in self.grid_cells.columns
        ):  # Replacing the residual availability column if it already exists in the grid_cells gdf to avoid confusion with the new availability analytics we are merging from the cutout.
            self.grid_cells.drop(columns=[avail_col], inplace=True)

        _avail_df = AvailabilityMatrix.rename(avail_col).to_dataframe().reset_index()
        ...
        self.availability_gdf = utils.assign_cell_id(
            cells=availability_gdf, source_column=self.sub_national_unit_tag
        )

        return self.availability_gdf

    ## Visuals
    def plot_ERA5_grid_land_availability(
        self,
        ...
        """
        availability_gdf = (
            availability_gdf if availability_gdf is not None else self.availability_gdf
        )
        region_boundary = (
            self.region_boundary if region_boundary is not None else self.region_boundary
        )
        availability_gdf[f"LandAvailability_ERA5_{self.resource_type}_pct"] = (
            availability_gdf[f"LandAvailability_ERA5_{self.resource_type}"] * 100
        )
        ...
        # Add shadow effects
        region_boundary_plot.geometry = region_boundary_plot.geometry.translate(
            xoff=shadow_offset, yoff=-shadow_offset
        )
        region_boundary_plot.plot(
            ax=ax, facecolor="none", edgecolor="gray", linewidth=2, alpha=0.3
        )  # Shadow layer

        availability_gdf_plot.plot(
            column=f"LandAvailability_ERA5_{self.resource_type}_pct",
            ax=ax,
            cmap="Greens",
            legend=True,
            legend_kwds={
        ...
        region_boundary_plot.plot(
            ax=ax, facecolor="none", edgecolor="black", linewidth=0.2, alpha=0.9
        )
        plt.subplots_adjust(right=0.85)  # Increase space on the right
        ax.set_title(
            f"Land Availability for {self.resource_type} resources ({self.region_name})",
            fontsize=14,
        )
        ...
        plt.savefig(
            f"{plot_save_to}/land_availability_ERA5grid_{self.region_short_code}_{self.resource_type}.png",
            dpi=500,
        )
        utils.print_update(
            level=PRINT_LEVEL_BASE + 3,
            message=f"{__name__}|Land availability (grid cells) map saved at {vis_save_to_root}",
        )

    def plot_excluder_land_availability(self, excluder: ExclusionContainer = None):
        """
        ...
        excluder.plot_shape_availability(
            geometry=region_shape, plot_kwargs={"facecolor": "none", "edgecolor": "black"}, ax=ax
        )
        ax.axis("off")
        vis_save_to_root = self.get_vis_dir()
        plot_save_to = Path(vis_save_to_root) / "lands"
        utils.ensure_path(plot_save_to)
        plt.savefig(
            f"{plot_save_to}/land_availability_excluderResolution_{self.region_name}.png", dpi=500
        )
        utils.print_update(
            level=PRINT_LEVEL_BASE + 3,
            message=f"{__name__}|Land availability map (excluder resolution) saved to {plot_save_to}/land_availability_excluderResolution_{self.region_name}.png",
        )
        plt.close()
        # return fig
...
def get_sub_nationally_aggregated_capacity(
    ...
    cols_to_aggregate = (
        capacity_cols
        + [area_col]
        + [col for col in cells_with_capacity.columns if col.startswith(developable_col_prefix)]
    )
    ...
    cells_aggr = (
        df.groupby(sub_national_unit_tag, dropna=not dropna_group).agg(agg_dict).reset_index()
    )
