...
def impute_ERA5_windspeed_to_Cells(
    cutout: atlite.Cutout, region_grid_cells: gpd.GeoDataFrame
) -> gpd.GeoDataFrame:
    ...
    utils.print_update(
        ...
        message=f"{__name__}| Calculating yearly mean windspeed and imputing to provincial Grid Cells named as 'windspeed_ERA5'",
    )

    # Calculate yearly mean windspeed
    wnd_ymean_df = (
        cutout.data.wnd100m.groupby("time.year")
        .mean("time")
        .to_dataframe(name="windspeed_ERA5")
        .reset_index()
    )

    # Create a GeoDataFrame for spatial join
    wnd_ymean_gdf = gpd.GeoDataFrame(
        wnd_ymean_df, geometry=gpd.points_from_xy(wnd_ymean_df["x"], wnd_ymean_df["y"])
    )
    wnd_ymean_gdf.crs = region_grid_cells.crs

    # Perform spatial join and drop unnecessary columns
    region_grid_cells = gpd.sjoin(
        region_grid_cells.rename(columns={"x": "x_dup", "y": "y_dup"}),
        wnd_ymean_gdf,
...
def scale_wind(row: pd.Series, wnd, method=2):
    ...
    if method == 1:
        scaled = (
            wind_at_location * row["GWA_wind_speed"] / np.mean(wind_at_location)
        )  # method1 , preferably for specific project points
    if method == 2:
        scaled = (
            wind_at_location * row["windspeed_gwa"] / np.mean(wind_at_location)
        )  # method2, preferably for ERA5 grid cells

    return scaled


def rescale_cutout_windspeed(cutout: atlite.Cutout, wind_assets: pd.DataFrame):
    ...
    nearest = cutout.data.sel(
        {"x": wind_assets.x.values, "y": wind_assets.y.values}, "nearest"
    ).coords
    ...
    scaled_wind_xy = (
        pd.DataFrame(data={"Wind": scaled_wind, "XY": xy}).dropna().reset_index()
    )  # .drop(columns='index')
    ...
    for i in range(scaled_wind_xy.index.size):
        ...
        to_data_array[:, scaled_wind_xy.loc[i].XY[1], scaled_wind_xy.loc[i].XY[0]] = (
            scaled_wind_xy.loc[i].Wind
        )
...
def get_speed(row, xaxis, yaxis, data):
    ...
    yIdx = len(yaxis) - np.searchsorted(
        yaxis, row["y"], side="left", sorter=np.arange(len(yaxis) - 1, -1, -1)
    )
...
def get_wind_coords(assets, wind_atlas, wind_geojson):
    ...
    longitudes = [
        wind_geojson[i][0][j][0]
        for i in range(len(wind_geojson))
        for j in range(len(wind_geojson[i][0]))
    ]  # [lon, lat], choose index 0
    latitudes = [
        wind_geojson[i][0][j][1]
        for i in range(len(wind_geojson))
        for j in range(len(wind_geojson[i][0]))
    ]  # [lon, lat], choose index 1
