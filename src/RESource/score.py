...
class CellScorer(AttributesParser):
    ...
    def get_CRF(self, r: float, N: int) -> float:
        """
    ...
    def calculate_total_cost_smooth_scaling(
        self,
        distance_to_grid_km: float,
        grid_connection_cost_per_km: float,
        tx_line_rebuild_cost: float,
    ...
    def calculate_total_cost_transmission_sizing(
        self,
        distance_to_grid_km: float,
        grid_connection_cost_per_km: float,
        tx_line_rebuild_cost: float,
        ...
        base_cost_adjusted = (
            distance_to_grid_km * grid_connection_cost_per_km * voltage_multiplier
    ...
    def calculate_total_cost_shared_infrastructure(
        self,
        distance_to_grid_km: float,
        grid_connection_cost_per_km: float,
        tx_line_rebuild_cost: float,
    ...
    def calculate_score(
        self, row: pd.Series, node_distance_col: str, CF_column: str, CRF: float
    ) -> float:
        """
    ...
    def calculate_score_debug(
        ...
        grid_cost = (
            row[node_distance_col] * row[f"grid_connection_cost_per_km_{self.resource_type}"]
    ...
    def get_cell_score(
        ...
        node_distance_col = utils.get_available_column(
            dataframe, ["nearest_station_distance_km", "nearest_distance"]
        )
        ...
        missing_columns = [
            col for col in required_columns if col is None or col not in dataframe.columns
        ]
        if missing_columns:
            raise AssertionError(
                f"Required columns {set(missing_columns)}or alternatives not found"
            )

        utils.print_update(
            level=PRINT_LEVEL_BASE + 2, message=f"{__name__}| Calculating score for cells..."
        )
        ...
        dataframe[f"lcoe_actualCap_{self.resource_type}"] = dataframe.apply(
            lambda row: self.calculate_score(row, node_distance_col, CF_column, CRF), axis=1
        )

        scored_dataframe: pd.DataFrame = dataframe.sort_values(
            by=f"lcoe_actualCap_{self.resource_type}", ascending=True
        ).copy()  # Lower LCOE is better (ascending=True)
    ...
    def calculate_score_per_mw(
        ...
        # LCOE per MW
        lcoe_millions = (
            (total_capex_per_mw * CRF) + fom_per_mw + vom_per_mw
        ) / annual_energy_per_mw
        lcoe_dollars = lcoe_millions * 1e6
