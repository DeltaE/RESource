...
class GADMBoundaries(AttributesParser):
    ...
    def __post_init__(self):
        ...
        self.region_file: Path = (
            Path(self.gadm_processed)
            / f"gadm41_{self.country}_L{self.admin_level}_{self.region_short_code}.geojson"
        )

    # Define a function to create bounding boxes (of cell) directly from coordinates (x, y) and resolution

    def get_country_boundary(
        self, country: str = None, force_update: bool = False
    ) -> gpd.GeoDataFrame:
        """
        ...
        """
        utils.print_update(
            level=PRINT_LEVEL_BASE + 1, message=f"{__name__} | Country Selected: {self.country}."
        )
        ...
        self.country_file: Path = (
            Path(self.gadm_root) / f"gadm41_{self.country}_L{self.admin_level}.geojson"
        )

        try:
            # Load or fetch data
            if (
                self.country_file.exists() and not force_update
            ):  # load the country gdf from local file
                utils.print_update(
                    level=PRINT_LEVEL_BASE + 1,
                    message=f"{__name__} | Loading GADM data for {self.country} from local datafile {self.country_file}.",
                )
                ...
                _country_gdf_: gpd.GeoDataFrame = pygadm.AdmItems(
                    name=self.country, content_level=self.admin_level
                )
            ...
            utils.print_update(
                level=PRINT_LEVEL_BASE + 1,
                message=f"{__name__} | Error fetching or loading GADM data: {e}",
            )
            raise

    def get_region_boundary(
        self, region_name: str = None, force_update: bool = False
    ) -> gpd.GeoDataFrame:
        """
        ...
        utils.print_update(
            level=PRINT_LEVEL_BASE + 2, message=f"{__name__}| Collecting regional boundary..."
        )
        ...
        if self.region_code_validity:
            ...
            if (
                self.region_file.exists() and not force_update
            ):  # There is a local file and no update required
                utils.print_update(
                    level=PRINT_LEVEL_BASE + 1,
                    message=f"{__name__}| Loading GADM boundaries (Sub-provincial | level =2) for {self.region_name} from local file {self.region_file}.",
                )
                ...
                if self.multi_country_flag:
                    utils.print_update(
                        level=PRINT_LEVEL_BASE + 1,
                        message=f"{__name__} | Processing the Boundaries for Multi-Country Region : {self.country}.",
                    )

                    _boundary_country = self.get_country_boundary(self.region_name, force_update)
                    _boundary_region_ = _boundary_country.loc[
                        _boundary_country["NAME_0"] == self.region_name
                    ]

                else:
                    _boundary_country = self.get_country_boundary(self.country, force_update)
                    _boundary_region_ = _boundary_country.loc[
                        self.boundary_country["NAME_1"] == self.region_name
                    ]

                if _boundary_region_.empty:
                    utils.print_update(
                        level=PRINT_LEVEL_BASE + 1,
                        message=f"{__name__}|  No data found for region '{self.region_name}'.",
                    )
                    utils.print_update(
                        level=PRINT_LEVEL_BASE + 1,
                        message=f"{__name__}| | @ LINE | Consider revising '{self.region_name}' to match source (e.g. https://gadm.org/maps.html); Select 'show sub-divisions' to get the list of Supported Regional Names",
                    )
                    raise ValueError(
                        f"{__name__} | @ LINE {inspect.currentframe().f_lineno} | No data found for region '{self.region_name}'."
                    )

                elif self.admin_level == 1:
                    _boundary_region_ = _boundary_region_[["NAME_0", "NAME_1", "geometry"]].rename(
                        columns={
                            "NAME_0": self.boundary_datafields["NAME_0"],
                            "NAME_1": self.boundary_datafields["NAME_1"],
                        }
                    )
                    self.boundary_region: gpd.GeoDataFrame = _boundary_region_

                else:
                    _boundary_region_ = _boundary_region_[
                        ["NAME_0", "NAME_1", "NAME_2", "geometry"]
                    ].rename(
                        columns={
                            "NAME_0": self.boundary_datafields["NAME_0"],
                            "NAME_1": self.boundary_datafields["NAME_1"],
                            "NAME_2": self.boundary_datafields["NAME_2"],
                        }
                    )
                    self.boundary_region: gpd.GeoDataFrame = _boundary_region_
                ...
                utils.print_update(
                    level=PRINT_LEVEL_BASE + 1,
                    message=f"{__name__}| GADM data for {self.region_name} saved to {self.region_file}.",
                )
            return self.boundary_region
        else:
            raise ValueError(
                f"{__name__} | @ LINE {inspect.currentframe().f_lineno} Invalid region code: {self.region_short_code}."
            )
    ...
    def get_bounding_box(self) -> tuple:
        ...
        """
        utils.print_update(
            level=PRINT_LEVEL_BASE + 1, message=f"{__name__}| Processing regional bounding box..."
        )
        ...
        utils.print_update(
            level=PRINT_LEVEL_BASE + 1,
            message=f"{__name__}| Setting up the Minimum Bounding Region (MBR) for {self.region_short_code}...",
        )
        min_x, min_y, max_x, max_y = self.actual_boundary.geometry.total_bounds

        # MBR=box(min_x, min_y, max_x, max_y)
        self.bounding_box: dict = {"minx": min_x, "maxx": max_x, "miny": min_y, "maxy": max_y}

        return self.bounding_box, self.actual_boundary

    def show_regions(
        self, basemap: str = "CartoDB positron", save_path: str = "vis/regions", save: bool = False
    ):
        """
    ...
    def run(self):
        ...
        if self.region_code_validity:
            ...
            utils.print_update(
                level=PRINT_LEVEL_BASE + 1, message=f"{__name__}| Region code is not valid."
            )
            return None
