...
def smooth_scaling(capacity_mw: float, reference_mw: float = 100.0, exponent: float = 0.8) -> float:
...
def run_grid_cost_sensitivity(
    ...
    for kappa in kappa_values:
        ...
        df_v["lcoe_variant"] = df_v.apply(
            lambda row, gcc_variant=gcc_variant: compute_lcoe(
    ...
    for kappa in kappa_values:
        ...
        merged = pd.merge(
            baseline_rank, df_v_sorted[["cluster_id", "rank_variant"]], on="cluster_id", how="inner"
        )
...
def plot_supply_curves(
    curves: dict, resource_type: str, save_path: Path, lcoe_threshold: float = 150.0
) -> None:
    ...
    for kappa, colour in zip(kappas, colours, strict=False):
        ...
        ax.step(cum_gw, lcoe, where="post", color=colour, linewidth=lw, linestyle=ls, label=lbl)
...
def plot_spearman(spearman_df: pd.DataFrame, resource_type: str, save_path: Path) -> None:
    """Bar plot of Spearman ρ vs. κ."""
    fig, ax = plt.subplots(figsize=(7, 4))
    ax.bar(
        spearman_df["kappa"].astype(str),
        spearman_df["spearman_rho"],
        color="steelblue",
        edgecolor="white",
        width=0.6,
    )
    ax.axhline(0.9, color="red", linestyle="--", linewidth=1.0, label="ρ = 0.90 reference")
    ax.set_xlabel("Cost multiplier κ", fontsize=11)
    ax.set_ylabel("Spearman ρ (rank correlation vs. baseline)", fontsize=11)
    ax.set_title(
        f"{resource_type.title()} – Rank stability across grid-cost scenarios", fontsize=12
    )
    ax.set_ylim(0, 1.05)
...
def main():
    ...
    parser.add_argument("--solar", type=Path, default=None, help="Path to solar cluster CSV")
    parser.add_argument("--wind", type=Path, default=None, help="Path to wind cluster CSV")
    parser.add_argument(
        "--gcc", type=float, default=2.6, help="Baseline grid-connection cost (M$/km); default 2.6"
    )
    parser.add_argument(
        "--tx", type=float, default=0.56, help="Baseline fixed rebuild cost (M$); default 0.56"
    )
    parser.add_argument("--rate", type=float, default=0.07, help="Discount rate; default 0.07")
    parser.add_argument(
        "--threshold",
        type=float,
        default=150.0,
    ...
    parser.add_argument(
        "--outdir", type=Path, default=Path("results/sensitivity/M2"), help="Output directory"
    )
    ...
    for resource_type, csv_path in [("solar", args.solar), ("wind", args.wind)]:
        ...
        # Save outputs
        results["spearman"].to_csv(args.outdir / f"M2_spearman_{resource_type}.csv", index=False)
        results["summary"].to_csv(args.outdir / f"M2_summary_{resource_type}.csv", index=False)
        ...
        plot_spearman(
            results["spearman"], resource_type, args.outdir / f"M2_spearman_{resource_type}.png"
        )
