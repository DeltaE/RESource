...
def recompute_lcoe_for_variant(
    df: pd.DataFrame,
    resource_type: str,
    gcc_baseline: float,
    tx_baseline: float,
...
def accessible_capacity_gw(
    df: pd.DataFrame, lcoe_series: pd.Series, density_multiplier: float, lcoe_threshold: float
) -> float:
...
def plot_tornado(
    oat_df: pd.DataFrame, resource_type: str, save_path: Path, lcoe_threshold: float
) -> None:
    """
    ...
    ax.set_title(
        ...
        f"(LCOE ≤ {lcoe_threshold:.0f} $/MWh threshold)",
        fontsize=12,
    )
    fig.tight_layout()
...
def plot_interaction_heatmap(
    interaction_df: pd.DataFrame, resource_type: str, save_path: Path
) -> None:
    """Heatmap of accessible GW across CAPEX × discount rate combinations."""
    pivot = interaction_df.pivot(index="rate_label", columns="capex_label", values="total_gw")
    # Ensure order
...
def main():
    ...
    parser.add_argument("--gcc", type=float, default=2.6, help="Baseline gcc (M$/km); default 2.6")
    parser.add_argument(
        "--tx", type=float, default=0.56, help="Baseline tx rebuild cost (M$); default 0.56"
    )
    parser.add_argument(
        "--rate", type=float, default=0.07, help="Baseline discount rate; default 0.07"
    )
    parser.add_argument(
        "--threshold",
        type=float,
    ...
    parser.add_argument("--outdir", type=Path, default=Path("results/sensitivity/M3"))
    ...
    for resource_type, csv_path in [("solar", args.solar), ("wind", args.wind)]:
        ...
        oat_df.to_csv(args.outdir / f"M3_oat_{resource_type}.csv", index=False)

        plot_tornado(
            oat_df, resource_type, args.outdir / f"M3_tornado_{resource_type}.png", args.threshold
        )
        ...
        interaction_df.to_csv(args.outdir / f"M3_interaction_{resource_type}.csv", index=False)
        ...
        print(
            oat_df[["parameter", "scenario_label", "total_gw", "delta_gw", "delta_pct"]].to_string(
                index=False
            )
        )
    ...
    if all_oat:
        pd.concat(all_oat).to_csv(args.outdir / "M3_oat_combined.csv", index=False)
    if all_interaction:
        pd.concat(all_interaction).to_csv(args.outdir / "M3_interaction_combined.csv", index=False)
