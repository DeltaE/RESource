...
class UtilityPvPlusBatteryProc(TechProcessor):
    ...
    def __init__(
        ...
        self.df_pv_cost: pd.DataFrame | None = None
        self.df_batt_cost: pd.DataFrame | None = None
...
class GeothermalProc(TechProcessor):
    ...
    def load_cff(
        ...
        assert len(df_cff) == len(cls.scenarios * 2), (
            f"Wrong number of CFF rows found. Expected {len(cls.scenarios) * 2}, get {len(df_cff)}."
        )
