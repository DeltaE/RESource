...
# import xlwings as xw # xlwings doesn't work in unix, EL :2025 04 18
from openpyxl import load_workbook  # alternative to xlwings, EL :2025 04 18

from .abstract_extractor import AbstractExtractor
from .config import FINANCIAL_CASES, TECH_DETAIL_SCENARIO_COL, YEARS, CrpChoiceType
...
class Extractor(AbstractExtractor):
    ...
    def __init__(
        ...
        wb = load_workbook(
            data_workbook_fname, data_only=True
        )  # alternative to xlwings, EL :2025 04 18
        sheet = wb["Financial and CRP Inputs"]
        sheet["B5"] = case
        sheet["E5"] = crp

        # wb.save() #xlwings
        wb.save(data_workbook_fname)  # openpyxl needs the fname args , EL :2025 04 18

        df = pd.read_excel(data_workbook_fname, sheet_name=sheet_name)
        df = df.reset_index()
        # Give columns numerical names
        columns = {x: y for x, y in zip(df.columns, range(0, len(df.columns)), strict=False)}
    ...
    def get_tax_credits_sheet(cls, data_workbook_fname):
        ...
        # Give columns numerical names
        columns = {x: y for x, y in zip(df_tc.columns, range(0, len(df_tc.columns)), strict=False)}
