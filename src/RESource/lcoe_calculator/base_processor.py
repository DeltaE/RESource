...
class TechProcessor(ABC):
    ...
    def __init__(
        ...
        assert case in FINANCIAL_CASES, (
            f"Financial case must be one of {FINANCIAL_CASES}, received {case}"
        )
        assert crp in CRP_CHOICES, f"Financial case must be one of {CRP_CHOICES}, received {crp}"
    ...
    def load_cff(
        ...
        assert len(df_cff) == len(cls.scenarios), (
            f"Wrong number of CFF rows found. Expected {len(cls.scenarios)}, get {len(df_cff)}."
        )
    ...
    def _calc_capex(self):
        assert self.df_cff is not None and self.df_occ is not None and self.df_gcc is not None, (
    ...
    def _calc_ptc(self):
        ...
        if self.has_tax_credit:
            ...
            assert len(df_ptc) == len(self.scenarios), (
