...
class NREL_ATBProcessor:
    ...
    def pull_data(self):
        ...
        utils.print_update(
            ...
            message=f"{__name__}| Processing Annual Technology Baseline (ATB) data sourced from NREL...",
        )
        ...
        utils.print_update(
            level=print_level_base + 1,
            message=f"{__name__}| ATB cost datafile: {self.atb_file_path.name} loaded",
        )

        utils.print_update(
            level=print_level_base + 1, message="Extracting technology baseline costs..."
        )
    ...
    def __process_solar_cost(self, atb_cost):
        ...
        """
        utils.print_update(
            level=print_level_base + 2, message="Extracting Solar PV technology cost..."
        )
        pv_cost_mask = (
            (atb_cost["technology_alias"] == "Utility PV")
            & (
                atb_cost["core_metric_parameter"].isin(
                    ["CAPEX", "Fixed O&M", "Variable O&M", "OCC"]
                )
            )
    ...
    def __process_wind_cost(self, atb_cost):
        ...
        """
        utils.print_update(
            level=print_level_base + 2, message="Extracting Wind Turbine technology cost..."
        )
        ...
        land_based_wind_cost = atb_cost[land_based_wind_cost_mask].sort_values(
