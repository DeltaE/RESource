...
class GridNodeLocator(AttributesParser):
    ...
    def __post_init__(self):
        ...
        self.grid_proximity_threshold_km: float = self.get_grid_proximity_km()
    ...
    def find_nearest_single_connection_point(
        self, cell_centroid, cell_geometry, cell_gdf, line_gdf
    ):
        """
        ...
        if not intersecting_lines.empty:
            ...
            candidate_cells = cell_gdf[
                cell_gdf.geometry.apply(
            ...
            candidate_cells["distance"] = candidate_cells["centroid"].apply(
                lambda x: x.distance(cell_centroid)
            )
            ...
            lines_to_search = lines_to_search[
                lines_to_search.geometry.type.isin(["LineString", "MultiLineString"])
            ]
        ...
        # 3. Find the nearest point on those lines
        distances = lines_to_search.geometry.apply(
            lambda line: cell_centroid.distance(line)
        ).astype(float)
    ...
    def find_nearest_connection_point(
        ...
        try:
            ...
            if not intersecting_lines.empty:
                intersecting_lines["geometry"] = intersecting_lines.geometry.intersection(
                    cell_geometry
                )
                lines_to_search = intersecting_lines
            else:
    ...
    def find_grid_nodes_ERA5_cells(
        self, buses_gdf: gpd.GeoDataFrame, cells_gdf: gpd.GeoDataFrame
    ) -> gpd.GeoDataFrame:
        """
        ...
        """
        _ = buses_gdf.sindex  # Generate and cache the spatial index.
        bus_tree = cKDTree(buses_gdf["geometry"].apply(lambda x: (x.x, x.y)).tolist())

        utils.print_update(
            level=PRINT_LEVEL_BASE, message="Calculating Nearest Grid Nodes for Grid Cells..."
        )
        ...
        cells_gdf[["nearest_station", "nearest_station_distance_km"]] = pd.DataFrame(
            result.tolist(), index=cells_gdf.index
        )
    ...
    def get_OSM_grid_lines(
        self, columns_to_keep: tuple[str, ...] = ("power", "voltage", "geometry")
    ) -> gpd.GeoDataFrame:
        """
        ...
        if "voltage" in lines_subset_gdf.columns:
            lines_subset_gdf["voltage"] = (
                pd.to_numeric(lines_subset_gdf["voltage"], errors="coerce").fillna(0).astype(int)
            )
